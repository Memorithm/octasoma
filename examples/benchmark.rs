...
fn evaluate(
    ...
    for (qi, (q, res)) in queries.iter().zip(&octree_results).enumerate() {
        ...
        if !pred.is_empty() && (pred[0] as usize).is_multiple_of(clusters) == q_cluster.is_multiple_of(clusters) {
            c1 += 1.0;
        }
        let same: usize = pred
            .iter()
            .filter(|&&p| (p as usize).is_multiple_of(clusters) == q_cluster.is_multiple_of(clusters))
            .count();
